...
// overflows in this method lead to valid results!!
ll random(ll a, ll b)
{
...
ll calc(ll val)
{
    ...
    if(val == 1)
        return res; // first case
    else if(isPrime(val))
        return res * 2; // second case
    else if(isSquare(val))
        return res * 3; // third case but with p == q
    else
        return res * 4; // third case with p != q
}
